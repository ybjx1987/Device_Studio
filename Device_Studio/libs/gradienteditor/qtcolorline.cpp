...
void QtColorLinePrivate::paintEvent(QPaintEvent *)
{
    ...
    if (q_ptr->isEnabled()) {
        if (m_backgroundCheckered) {
            ...
            QRegion regin(r[1].adjusted(4, 4, -4, -4));
            regin = regin.united(QRect(rect.topLeft(), QPoint(r[1].left() + 0, rect.bottom())));
            regin = regin.united(QRect(QPoint(r[1].right() - 0, rect.top()), rect.bottomRight()));
            regin = regin.united(QRect(rect.topLeft(), QPoint(rect.right(), r[1].top() + 0)));
            regin = regin.united(QRect(QPoint(rect.left(), r[1].bottom() - 0), rect.bottomRight()));
            p.setClipRegion(regin);
...
*/
            ...
            if (m_orientation == Qt::Horizontal) {
                ...
                region = region.united(r[1].adjusted(0, 0, 0, -qRound(r[1].height() * coef)));
                p.setClipRegion(region);
            } else {
                QRegion region(r[1].adjusted(qRound(r[1].width() * coef), 0, 0, 0));
                region = region.united(r[1].adjusted(0, 0, -qRound(r[1].width() * coef), 0));
                p.setClipRegion(region);
            }
            p.setBrush(Qt::NoBrush);
